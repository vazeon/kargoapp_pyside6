...
class ZoomTableMixin:
    ...
    def _lebar_kolom_aman(cls, value, default=100) -> int:
        ...
        try:
            ...
            value = int(default)
        return max(cls.MIN_COLUMN_WIDTH, min(value, cls.MAX_COLUMN_WIDTH))

    def _zoom_key(self, zoom_key: Optional[str] = None) -> str:
        """Menentukan key zoom tanpa mengikat mixin ke modul tertentu."""
        key = zoom_key or getattr(self, "ZOOM_KEY", None)
        return str(key).strip() if key else self.__class__.__name__
    ...
    def _set_style_dasar_zoom(self, widget: QWidget, stylesheet: str) -> None:
