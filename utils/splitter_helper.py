...
def _deteksi_mode_gelap(widget: QWidget | None) -> bool:
    ...
    if window is not None and hasattr(window, "current_theme"):
    ...
    *widgets: QWidget,
    ...
    if ukuran_awal is not None:
        ...
        if len(ukuran) != len(widgets):
            raise ValueError(
                "Jumlah ukuran_awal harus sama dengan jumlah widget splitter."
            )

        splitter.setSizes(ukuran)

    is_dark = _deteksi_mode_gelap(parent)
    splitter.setStyleSheet(get_splitter_style(is_dark))

    return splitter


def perbarui_semua_style_splitter(
    root: QWidget,
    is_dark: bool,
) -> None:
