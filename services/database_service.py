...
def ambil_data_buku_gudang(kode_cabang, wilayah, tahun_terpilih, filters=None):
    ...
    query = """
        SELECT no_resi, tanggal_masuk, tanggal_keluar, status_resi, truk,
               pengirim, kota_asal, penerima, kota_tujuan, nama_barang,
               koli, berat, cbm, total_ongkir, pembayaran, ket_buku_gudang
        FROM data_resi
        WHERE kode_cabang = ?
    ...
    for col_idx, val in (filters or {}).items():
...
def simpan_semua_pengaturan_dan_cabang(settings_to_save, branches_to_save):
    ...
    try:
        ...
        seen_codes = set()
        cabang_aktif = str(CURRENT_SESSION.get("kode_cabang", "")).strip().upper()
